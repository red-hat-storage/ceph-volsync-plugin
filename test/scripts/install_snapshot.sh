...
SNAPSHOT_VERSION=${SNAPSHOT_VERSION:-"v8.5.0"}
...
function create_or_delete_resource() {
    ...
    # always disable group snapshot
    if [ "${operation}" == "create" ]; then
        ARGUMENT="--feature-gates=CSIVolumeGroupSnapshot=false"
        if grep -q -E \
            "^\s+-\s+--feature-gates=CSIVolumeGroupSnapshot=" \
            "${temp_snap_controller}"; then
            sed -i -E \
                "s|^(\s+-\s+)--feature-gates=CSIVolumeGroupSnapshot=.*$|\1$ARGUMENT|" \
